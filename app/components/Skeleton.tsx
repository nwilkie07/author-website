...
export const SkeletonImage: React.FC<BaseProps> = ({
  height = 240,
  width = "100%",
}) => {
  return (
...
export const SkeletonLine: React.FC<BaseProps> = ({
  height = 16,
  width = "100%",
}) => {
  return (
...
export const SkeletonTextBlock: React.FC<{ lines?: number }> = ({
  lines = 2,
}) => {
  return (
    <div className="flex flex-col gap-2 w-full">
      {Array.from({ length: lines }).map((_, i) => (
        <SkeletonLine key={i} height={20} width={"100%"} />
      ))}
    </div>
  );
};
